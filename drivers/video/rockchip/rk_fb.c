...
#elif defined (CONFIG_TCHIP_LOGO_ROTATE_180)
		if(fb_prepare_logo(fb_inf->fb[0], FB_ROTATE_UD)) {
		       /* Start display and show logo on boot */
		       fb_set_cmap(&fb_inf->fb[0]->cmap, fb_inf->fb[0]);
		       fb_show_logo(fb_inf->fb[0], FB_ROTATE_UD);
		       fb_inf->fb[0]->fbops->fb_pan_display(&(fb_inf->fb[0]->var), fb_inf->fb[0]);
		}
#elif defined (CONFIG_TCHIP_LOGO_ROTATE_90) && defined (CONFIG_LOGO_LINUX_CLUT224) && defined (CONFIG_TCHIP_MACH_TR072)
		if(fb_prepare_logo(fb_inf->fb[0], FB_ROTATE_CW)) {
			/* Start display and show logo on boot */
			fb_set_cmap(&fb_inf->fb[0]->cmap, fb_inf->fb[0]);
			fb_show_logo(fb_inf->fb[0], FB_ROTATE_CW);
			fb_inf->fb[0]->fbops->fb_pan_display(&(fb_inf->fb[0]->var), fb_inf->fb[0]);
		}
#elif defined (CONFIG_TCHIP_LOGO_ROTATE_270) && defined (CONFIG_LOGO_LINUX_CLUT224) && defined (CONFIG_TCHIP_MACH_TR072)
		if(fb_prepare_logo(fb_inf->fb[0], FB_ROTATE_CCW)) {
			/* Start display and show logo on boot */
			fb_set_cmap(&fb_inf->fb[0]->cmap, fb_inf->fb[0]);
			fb_show_logo(fb_inf->fb[0], FB_ROTATE_CCW);
			fb_inf->fb[0]->fbops->fb_pan_display(&(fb_inf->fb[0]->var), fb_inf->fb[0]);
		}
#else
