...
{
    int cal_brightness;
    DBG("%s: rk29_bl_info->max_brightness=%d\n", __FUNCTION__, rk29_bl_info->max_brightness);

    if (rk29_bl_info->max_brightness < rk29_bl_info->min_brightness)
        rk29_bl_info->max_brightness = rk29_bl_info->min_brightness;

    // 鎶婁笂灞備紶涓嬫潵鐨�1~255)鐨勫�姣斾緥瀵瑰簲鍒�rk29_bl_info->min_brightness ~ rk29_bl_info->max_brightness
...
//BL_CORE_DRIVER1 is the flag if backlight pwm is closed.
	...
	if (xhc_flag == 0) {
        msleep(100);
        xhc_flag = 1;
        gpio_direction_output(RK30_PIN0_PA2, GPIO_HIGH); 
    }

	printk("%s:line=%d,brightness = %d, div_total = %d, divh = %d state=%x \n",__FUNCTION__,__LINE__,brightness, div_total, divh,bl->props.state);
out:
