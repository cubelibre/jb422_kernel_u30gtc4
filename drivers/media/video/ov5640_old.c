	...
	if (sensor_af_init(client)) {
        ...
        if (sensor_fmt_videochk(NULL, &mf) == true) {    /* ddl@rock-chips.com: focus mode fix const auto focus in video */
    		...
    		switch (sensor->info_priv.auto_focus)
    		{
    			...
    			{
    				printk("SENSOR_AF_MODE_AUTO---------xhc-------\n");
		...
		case Sensor_Flash:
		{
			...
			if (sensor->sensor_io_request && sensor->sensor_io_request->sensor_ioctrl) {
                ...
                if(on == Flash_On){
            		...
            		hrtimer_start(&(flash_off_timer.timer),ktime_set(0, 2*1000*1000*1000),HRTIMER_MODE_REL);
                    }
			}
			break;
		}
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	//ret = sensor_write(client, 0x3103, 0x11);
   // if (ret != 0) {
   //     SENSOR_TR("%s soft reset sensor 3103 failed\n",SENSOR_NAME_STRING());
   //     ret = -ENODEV;
	//	goto sensor_INIT_ERR;
   // }
