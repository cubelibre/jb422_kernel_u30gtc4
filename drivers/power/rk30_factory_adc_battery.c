...
#define BATT_FILENAME "/oem/bat_last_capacity.dat"
...
#else
#define BATT_MAX_VOL_VALUE                              8290              	//Full charge voltage
...
static int batt_table[2*BATT_NUM+6] =
{
	...
	//6713, 7133, 7259 ,7310 ,7366 ,7432, 7530 ,7654 ,7785 ,7955 ,8200 ,//discharge
	6800, 7257, 7364, 7410, 7443, 7514, 7604, 7727, 7865, 8025, 8290,  //1
	//6800, 7255, 7364, 7417, 7444, 7514, 7604, 7725, 7871, 8025, 8299,  //2
	//6800, 7257, 7364, 7409, 7442, 7514, 7603, 7731, 7860, 8025, 8286,  //3
      //7232 ,7572, 7710 ,7745 ,7805 ,7898, 8003, 8135 ,8312, 8339, 8343 //charge
	//7132 ,7550, 7670 ,7738 ,7780 ,7845, 7930, 8030 ,8150, 8280, 8320 //charge
        7150, 7544, 7680, 7715, 7755, 7825, 7905, 8105, 8146, 8316, 8336   //1  7170 8015 第二版
        //6783, 7564, 7685, 7725, 7781, 7875, 7984, 8132, 8326, 8336, 8346   //2
	//6839, 7569, 7693, 7725, 7785, 7873, 7983, 8130, 8326, 8336, 8341   //3
};
#define adc_to_voltage(adc_val) ((adc_val * BAT_DEFINE_VALUE * (batt_table[4] +batt_table[5])) / (1024 *batt_table[5]))
#endif
...
static struct device_attribute rkbatt_attrs[] = {
	...
	__ATTR(value, 0777, rkbatt_show_value_attrs, rkbatt_restore_value_attrs),
	__ATTR(flag,  0777, rkbatt_show_flag_attrs,  rkbatt_restore_flag_attrs),
};
	...
	//charging 	    
		if( is_charge_ok(bat)  ==  INVALID_CHARGE_CHECK){
			if (bat->bat_capacity == 100){
				...
				{
					if(get_seconds() - bat->time_to_full > 600){
...
//#endif
	...
	//chargeing state
		...
		if( bat  ->pdata->is_reboot_charging == 1)
			...
			bat ->bat_capacity = ((new_capacity > old_capacity)&&(new_capacity < old_capacity+3)) ? new_capacity : old_capacity;
	}else{
		if(new_capacity > old_capacity + 50 )
			bat ->bat_capacity = new_capacity;
		else
			bat ->bat_capacity = ((new_capacity < old_capacity)&&(new_capacity > old_capacity-3)) ? new_capacity : old_capacity;  //avoid the value of capacity increase 
		if(bat->bat_capacity == 100)
			bat->bat_capacity = 99;
		if(bat->bat_capacity == 0)
			bat->bat_capacity =1;
...
//	struct rk30_adc_battery_data *bat = gBatteryData;
	...
	//chargeing state
		...
		bat->bat_capacity = ((new_capacity < old_capacity)&&(new_capacity > old_capacity-3)) ? new_capacity : old_capacity;  // aviod the value of capacity increase    dicharge
	}
		...
		if (++AdcTestCnt >= 2)
			{
				...
				if (fd_log < 0) {
						...
						sprintf(buf," [%02d:%02d:%02d] Status = %d, RealAdcVal = %d, RealVol = %d,gBatVol = %d, gBatCap = %d, RealCapacity = %d, batt_dischargecnt = %d\n,  chargecnt = %d,ac_count = %d, usb_count =%d ,usb_dischargecount =%d\n", 
