					...
					const char *_buf, size_t _count)
{
	...
	else if(!strncmp(_buf, "set", strlen("set")))
    {
        p+=strlen("set");
        if(*p-48)
        {
                snd_soc_write(es8323_codec, ES8323_CHIPPOWER, 0x00);    
                snd_soc_write(es8323_codec, ES8323_DACPOWER, 0x0c);     
                    snd_soc_write(es8323_codec, ES8323_ADCPOWER, 0x59);
            
                    //es8323_resume(es8323_codec);
                    //len = sprintk(count,"es8323 resume...\n");
            }else{
                    snd_soc_write(es8323_codec, ES8323_ADCPOWER, 0xff);
                    snd_soc_write(es8323_codec, ES8323_DACPOWER, 0xf0);
                    snd_soc_write(es8323_codec, ES8323_DACPOWER, 0xc0);
                    ...
                    //es8323_suspend(es8323_codec,0);
                    //len = sprintf(count,"es8323 suspend...\n");
            }
    }
