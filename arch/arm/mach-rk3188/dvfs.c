...
static struct lkg_maxvolt lkg_volt_table[] = {
	...
	{.leakage_level = 3,	.maxvolt = 1300 * 1000},
...
void dvfs_adjust_table_lmtvolt(struct clk *clk, struct cpufreq_frequency_table *table)
{
    ...
    if(have_leakage_flag)
    {
        unsigned long delayline_val = 0;
        unsigned long high_delayline = 0, low_delayline = 0,max_delayline=0;
        unsigned long rate_nandc = 0;
        rate_nandc = clk_get_rate(clk_get(NULL, "nandc")) / KHZ;
        high_delayline = HIGH_DELAYLINE * 148500 / rate_nandc;
        low_delayline = LOW_DELAYLINE * 148500 / rate_nandc;
        max_delayline = MAX_DELAYLINE * 148500 / rate_nandc;
        delayline_val = rk30_get_avs_val();
        printk("delayline check,fix chip's leakage is wrong,val = %lu.(HDL=%lu, LDL=%lu,MAL=%lu)\n",delayline_val, high_delayline, low_delayline,max_delayline);
        if((leakage_level<4)&&(delayline_val <= low_delayline)){
            leakage_level = 1;
            printk("Delayline too low, reset leakage_level \n");
        }else if((leakage_level>3&&leakage_level<6)&&(delayline_val < max_delayline)){
            leakage_level = 2;
            printk("Delayline too high, reset leakage_level \n");
        }
    }
